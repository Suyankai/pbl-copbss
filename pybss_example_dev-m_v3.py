...
if __name__ == '__main__':
    ...
    hop = 0#Initiate the number of hops
    ...
    for source_number in np.arange(2, 5, 1):
        ...
        for test_i in np.arange(1, 3, 1):
            ...
            print('MeICA:   ', Eval_dB, '; ', time, 'Average Time', time_hat1,' Hops: 1')

            # Time and accuracy of MeICA_dev
            # 添加对meica_dev时间的测量
            pyfbss_tb.timer_start()
            hat_S, hop = pyfbss.meica_dev(X, max_iter=100, tol=1e-04, break_coef=0.92, ext_multi_ica=extraction_base)
            time = pyfbss_tb.timer_value()
            time_hat2 = time / hop
            Eval_dB = pyfbss_tb.bss_evaluation(S, hat_S, eval_type)
            tmp_meicadev.extend([Eval_dB, time])
            print('MeICA_dev:   ', Eval_dB, '; ', time, 'Average Time', time_hat2,' Hops:', hop)
