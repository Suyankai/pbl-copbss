...
def run_plot(if_fast: bool, if_me: bool, if_simu: bool, latency_ms, latency_serv, performance):
    ...
    for s in PGB(range(2, source_number+1, 1)):
        ...
        if if_fast:
            ...
            snr_list_FastICA.append(pyfbss_tb.bss_evaluation(
        ...
        if if_me:
            ...
            snr_list_MeICA.append(pyfbss_tb.bss_evaluation(
    ...
    plt.plot(np.array(source_list), np.array(
        time_list_ms_MeICA), 'o-', label='MircoService_MeICA')
    if if_fast:
        plt.plot(np.array(source_list), np.array(
            time_list_FastICA), 'o-', label='FastICA')
    if if_me:
        plt.plot(np.array(source_list), np.array(
    ...
    plt.subplot(1, 2, 2)
    plt.plot(np.array(source_list), np.array(
        snr_list_ms_MeICA), 'o-', label='MircoService_MeICA')
    if if_fast:
        plt.plot(np.array(source_list), np.array(
            snr_list_FastICA), 'o-', label='FastICA')
    if if_me:
        plt.plot(np.array(source_list), np.array(
